...
use crate::config_handler::dirs;
use log::{LevelFilter, Record};
use log4rs;
use log4rs::append::{console::ConsoleAppender, file::FileAppender, Append};
use log4rs::config::{Appender, Config, Logger, Root};
use log4rs::encode::pattern::PatternEncoder;
use log4rs::encode::writer::simple::SimpleWriter;
use log4rs::encode::Encode;
use log4rs::file::{Deserializers, Deserialize};
use std::error::Error;
...
///
/// For further details, see the [module docs](index.html).
pub fn init_with_output_file(
    show_thread_name: bool,
    output_file_name_override: Option<String>,
) -> Result<(), String> {
    init_once_guard(|| init_impl(show_thread_name, true, output_file_name_override))
}
...
pub fn init_impl(
    ...
    let mut path = dirs().map_err(|e|format!("{:?}", e))?.config_dir().to_path_buf();
    path.push("log.toml");
    if let Ok(log_config_file) = File::open(path.into()) {
        ...
        return log4rs::init_file(path, deserializers).map_err(|e| format!("{}", e))
    }
    ...
    if with_output_file {
        ...
        let config = Config::builder()
            ...
            .build(Root::builder().appender("console").build(default_level))
            .map_err(|e| format!("{}", e))?;
...
impl Deserialize for AsyncFileAppenderCreator {
    ...
    fn deserialize(
        ...
        let mut path = dirs().map_err(|e|format!("{:?}", e))?.config_dir().to_path_buf();
        path.push("log.toml");
        let op_path = File::open(path.into()) {
...
impl AsyncAppender {
    fn new<W: 'static + SyncWrite + Send>(mut writer: W, encoder: Box<dyn Encode>) -> Self {
        ...
        let joiner = unwrap!(std::thread::Builder::new()
